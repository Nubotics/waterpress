    ...
    entity: function entity(user) {
      if (user) {
        ...
        if (_coreUtil.has(user, 'metaObj')) {
          ...
          if (_coreUtil.has(user.metaObj, 'nio_avatar')) {
  ...
  category: {
    ...
    detailCollection: function detailCollection(collection, termCollection) {
      ...
      _coreUtil.forEach(collection, function (ogItem) {
        ...
        termItem = _coreUtil._.find(termCollection, { id: ogItem.term.id });
        if (termItem) {
          newChildCollection = [];

          //-> if -> has -> childCollection
          if (_coreUtil.has(ogItem, 'childCollection') && _coreUtil.has(termItem, 'childCollection')) {
            ...
            _coreUtil.forEach(ogItem.childCollection, function (ogChildItem) {
              ...
              detailChildItem = _coreUtil._.find(termItem.childCollection, { id: ogChildItem.id });
              if (detailChildItem) {
          ...
          var slug = termItem.slug;
          var _name = termItem.name;
          var group = termItem.group;
    ...
    collectionWithChildren: function collectionWithChildren(collection) {
      var newCollection = [];
      var newItem = {};
      _coreUtil.forEach(collection, function (item) {
        var term = item.term;
