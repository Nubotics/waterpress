...
var checkHasPassport = function checkHasPassport(userObj) {
  var result = false;
  if (_coreUtil.has(userObj, 'passportCollection')) {
...
var guaranteePassword = function guaranteePassword(userObj) {
  ...
  if (_coreUtil.has(userObj, 'password')) {
...
var assembleUser = function assembleUser(userObj) {
  ...
  proxyUser.metaCollection = _coreUtil.assign(proxyUser.metaCollection, userObj.metaCollection);
