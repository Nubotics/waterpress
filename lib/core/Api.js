...
var Api = (function (_EventApi) {
  ...
  function Api(options) {
      ...
      _util.forEach(pluginCollection, function (plugin) {
        if (_util.has(plugin, 'name')) {
          ...
          if (_util.has(plugin, 'override')) {
            //--> override models
            if (_util.has(plugin.override, 'model')) {
              if (plugin.override.model) {
                console.log('plugin ' + plugin.name + ' model override', plugin.override.model);
                _util.eachKey(plugin.override.model, function (overModelKey) {
                  var _merge;

                  overrideModelCollection = _util.merge(overrideModelCollection, (_merge = {}, _merge[overModelKey] = plugin.override.model[overModelKey], _merge));
                });
              }
            }
            //--> override apis
            if (_util.has(plugin.override, 'api')) {
              if (plugin.override.api) {
                _util.eachKey(plugin.override.api, function (overApiKey) {
                  var _merge2;

                  overrideApiCollection = _util.merge(overrideApiCollection, (_merge2 = {}, _merge2[overApiKey] = plugin.override.api[overApiKey], _merge2));
                });
              }
            }
          }
    ...
    var mergeModelOverrides = function mergeModelOverrides(original, override) {
      _util.eachKey(original, function (ogKey) {
        if (_util.has(override, ogKey)) {
          original[ogKey] = _util.merge(original[ogKey], override[ogKey]);
        }
      });
      return original;
    };
    ...
    _util.eachKey(_models2['default'], function (modKey) {
      if (_util.has(overrideModelCollection, modKey)) {
        ...
        modelCollection = _util.merge(modelCollection, (_merge3 = {}, _merge3[modKey] = _models2['default'][modKey], _merge3));
      }
    });
    ...
    _util.eachKey(_api2['default'], function (apiKey) {
      if (_util.has(overrideApiCollection, apiKey)) {
        ...
        apiCollection = _util.merge(apiCollection, (_merge4 = {}, _merge4[apiKey] = _api2['default'][apiKey], _merge4));
      }
    });
  ...
  Api.prototype.end = function end(cb) {
    this.chain(function (next) {
      if (cb) {
        cb(next);
      } else {
