...
var postApi = {
  all: function all(params, limit, sort, chainStart, chainEnd, cb) {
    var _this = this;

    var action = function action(collections) {

      var query = collections.post.find().where(params);
      ...
      query.populate('author').populate('relationshipCollection').populate('metaCollection').exec(function (e, posts) {
