  ...
  if (this.collections) {
    ...
    this.collections.termtaxonomy.find().where(params).populate('term').populate('relationshipCollection').exec(function (e, termTaxCollection) {
      ...
      (0, _util.map)(termTaxCollection, function (termTax) {
        if (contains(termTax.term.slug, format) || contains(termTax.term.name, format)) {
          filterResult.push(termTax);
        }
      });
      var postIdCollection = [];
      if (filterResult.length > 0) {
        (0, _util.map)(filterResult, function (filterTermTax) {
          if ((0, _util.has)(filterTermTax, 'relationshipCollection')) {
            (0, _util.map)(filterTermTax.relationshipCollection, function (relation) {
...
var findByCategory = function findByCategory(category, options, cb, next) {
  ...
  if (this.collections) {
    ...
    this.collections.termtaxonomy.find().where(params).populate('term').populate('relationshipCollection').exec(function (e, termTaxCollection) {
      var filterResult = [];
      (0, _util.map)(termTaxCollection, function (termTax) {
        if (contains(termTax.term.slug, category) || contains(termTax.term.name, category)) {
          filterResult.push(termTax);
        }
      });
      var postIdCollection = [];
